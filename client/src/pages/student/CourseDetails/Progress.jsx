...
const Progress = ({ courseCode, courseId }) => {
  ...
  const PerformanceBarTooltip = useCallback(({ active, payload, label }) => {
    ...
    try {
      const data = payload[0].payload;
      const performanceValue = data.performance_percentage;
      
      return (
        <div className="bg-gray-900 border border-gray-600 p-4 rounded-lg shadow-xl backdrop-blur-sm">
          <p className="text-white font-semibold mb-2">{`Week ${safeNumber(label, 1)}`}</p>
          <p className="text-purple-400">
            Performance: {performanceValue !== null ? `${performanceValue.toFixed(1)}%` : 'N/A (No assignments)'}
          </p>
    ...
    try {
      const processedData = weeklyPerformance.weekly_performance
        ...
        .map((week, index) => {
          ...
          let performancePercentage = null; // Use null for weeks with no assignments
          if (possiblePoints > 0) {
            performancePercentage = Math.min(100, Math.max(0, (earnedPoints / possiblePoints) * 100));
          } else if (totalAssignments > 0) {
            // If there are assignments but no points possible, still calculate based on completion
            performancePercentage = gradedAssignments === totalAssignments ? 100 : 0;
          }
          // If no assignments at all, leave as null (will show as N/A)
          ...
          const result = {
            ...
            performance_percentage: performancePercentage !== null ? Math.round(performancePercentage * 100) / 100 : null,
            ...
            has_assignments: totalAssignments > 0 // Flag to indicate if week has assignments
          };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-gray-900/50 rounded-lg p-6 border border-gray-700">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart 
                    data={trendData.map(week => ({
                      ...week,
                      // Show 0 for weeks with no assignments but mark them specially
                      display_percentage: week.performance_percentage !== null ? week.performance_percentage : 0,
                      has_no_assignments: week.performance_percentage === null
                    }))} 
                  ...
                  >
                    ...
                    >
                      {trendData.map((entry, index) => (
                        <Cell 
                          key={`cell-${index}`} 
                          fill={entry.performance_percentage === null ? '#6B7280' : 'url(#barGradient)'}
                          stroke={entry.performance_percentage === null ? '#9CA3AF' : 'none'}
                          strokeDasharray={entry.performance_percentage === null ? '4 4' : 'none'}
                        />
                      ))}
                    </Bar>
                  </BarChart>
              ...
              <div className="mt-4 flex items-center justify-center gap-6 text-sm">
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 bg-gradient-to-b from-purple-500 to-purple-700 rounded"></div>
                  <span className="text-gray-300">Performance %</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 bg-gray-500 rounded border border-gray-400 border-dashed"></div>
          ...
          <div className="bg-gray-900/50 rounded-lg p-6 border border-gray-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {weeklyPerformance.weekly_performance.map((week) => {
                const performancePercentage = week.performance_percentage !== null ? safeNumber(week.performance_percentage) : null;
                const earnedPoints = safeNumber(week.total_earned_points);
                const possiblePoints = safeNumber(week.total_possible_points);
                const gradedAssignments = safeNumber(week.graded_assignments);
                const totalAssignments = safeNumber(week.total_assignments);
                  ...
                  <div key={week.week_number} className="bg-gradient-to-br from-gray-800 to-gray-700 rounded-lg p-4 border border-gray-600 hover:border-gray-500 transition-all duration-300">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span className={`text-sm font-bold px-2 py-1 rounded ${
                        performancePercentage !== null ? getGradeColor(performancePercentage) : 'text-gray-400 bg-gray-600'
                      }`}>
                        {performancePercentage !== null ? `${performancePercentage.toFixed(1)}%` : 'N/A'}
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-gray-400">
                        ...
                        <span className="text-white">
                          {totalAssignments > 0 ? `${earnedPoints.toFixed(1)}/${possiblePoints.toFixed(1)}` : 'No assignments'}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs text-gray-400">
                        <span>Assignments:</span>
                      ...
                      <div className="w-full bg-gray-600 rounded-full h-2 mt-2">
                        <div 
                          className={`h-2 rounded-full transition-all duration-300 ${
                            performancePercentage === null ? 'bg-gray-500' :
                          ...
                          style={{ 
                            width: performancePercentage !== null ? 
                              `${Math.min(100, Math.max(0, performancePercentage))}%` : 
                              '100%' 
                          }}
                        ></div>
                      </div>
                      {performancePercentage === null && (
                        <p className="text-xs text-gray-500 text-center mt-1">No assignments this week</p>
                      )}
                    </div>
                  </div>
                );
